...
// ---- 背景图文件存储：图片落盘到 DSH_HOME/storages/dsh-skins/backgrounds，配置只存路径 ----
const BG_DIR = join(DSH_HOME, 'storages', 'dsh-skins', 'backgrounds')
const MIME_EXT = {
  'image/png': '.png',
  'image/jpeg': '.jpg',
  'image/webp': '.webp',
...
/** `/dsh-skins` 通道：saveBackground / readBackground / deleteBackground。 */
async function rpcHandler(endpoint, payload) {
  ...
  if (endpoint === 'saveBackground') {
    ...
    try {
      ...
      // 内容寻址文件名：同一张图重复保存复用同一文件，避免缓存堆积。
      const file = `bg-${createHash('sha1').update(info.base64).digest('hex').slice(0, 12)}${ext}`
      const target = join(BG_DIR, file)
      if (!existsSync(target)) writeFileSync(target, Buffer.from(info.base64, 'base64'))
  ...
  if (endpoint === 'deleteBackground') {
    const target = safeBgPath(input.path)
    if (target === null) return { ok: false, error: { code: 'bad-request', message: '非法路径' } }
    try {
      if (existsSync(target)) unlinkSync(target)
      return { ok: true, value: { deleted: true } }
    } catch (error) {
      return { ok: false, error: { code: 'delete-failed', message: String(error && error.message || error) } }
    }
  }
...
export function apply(ctx) {
  ...
  // 背景图存取通道：connection 服务就绪后注册（ctx.inject 不阻塞 apply，
  // 缺失时仅跳过，客户端降级为预设/URL）。loopback 信任域。
  ctx.inject(['connection'], (connectionCtx) => {
    const dispose = connectionCtx.connection.rpc.handle('/dsh-skins', rpcHandler, { authority: 'loopback' })
