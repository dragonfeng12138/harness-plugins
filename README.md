...
「字体」页签与主题/皮肤完全独立：正文字体（`--dsw-font-family`）与代码字体（`--ds-font-family-code`）分开选择。字体列表**只显示本机已安装的字体**——通过 Canvas 宽度比对自动探测（无权限弹窗、无网络请求），并可一键重新检测。选定后以 `!important` 覆盖主题/皮肤自带的字体；选「跟随主题 / 皮肤」则交还。选择经 localStorage 持久化。
