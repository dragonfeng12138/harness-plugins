...
主题页签内还可设置**背景图像**：5 款内置原创 SVG 预设（星野银河 / 极光天幕 / 樱花空 / 像素夕阳 / 夜城霓虹，离线可用），或自定义图片 URL / 本地文件（≤3.5MB）。**本地图片由 host 落盘到 `DSH_HOME/dsh-skins/backgrounds/`，配置/设置文档只存文件路径（不存图像数据）**；加载时按路径读回，**文件不存在时自动回退第一个预设**。背景为**全屏工作区背景**（不改变窗口尺寸、无留边），工作区各背景层（框架 / 面板 / 侧栏）按主题色玻璃化，**「毛玻璃透明度」滑杆（0–100%）**控制透出程度；明暗切换自动重算玻璃色。激活皮肤时皮肤自带背景优先，回到主题轨道自动恢复。
