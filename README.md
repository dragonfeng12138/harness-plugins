...
主题页签内还可设置**背景图像**：5 款内置原创 SVG 预设（星野银河 / 极光天幕 / 樱花空 / 像素夕阳 / 夜城霓虹，离线可用），或自定义图片 URL / 本地文件（≤3.5MB，存于浏览器本地）。背景生效时应用框架变成毛玻璃面板（`color-mix` 半透明 + 背景模糊），颜色随主题明暗自适应；激活皮肤时皮肤自带背景优先，回到主题轨道自动恢复。
